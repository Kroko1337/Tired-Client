...
import beta.tiredb56.event.events.JumpEvent;
import beta.tiredb56.module.impl.list.combat.KillAura;
import beta.tiredb56.module.impl.list.visual.SlowerHit;
...
public abstract class EntityLivingBase extends Entity {
    ...
    public Vec3 getLook(float partialTicks) {
        float pitch = 0, yaw = 0, prevPitch, prevYaw;
        final ScaffoldWalk scaffoldWalk = CheatMain.INSTANCE.moduleManager.moduleBy(ScaffoldWalk.class);


        if (!scaffoldWalk.isState() && !KillAura.getInstance().isState()) {
            yaw = rotationYaw;
            pitch = rotationPitch;
        }

        if (KillAura.getInstance().isState()) {
            yaw = Rotations.server_yaw;
            pitch = Rotations.server_pitch;
        }
