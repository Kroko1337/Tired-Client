...
import beta.tiredb56.event.events.EventLook;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public Vec3 getLook(float partialTicks) {
        if ((KillAura.getInstance().isState() && KillAura.getInstance().rayCast.getValue()) || (ScaffoldWalk.getInstance().isState())) {
    ...
    public void onUpdate() {
        ...
        if (PerformanceGui.usingType == UsingType.HIGH_PERFORMANCE) {
            CheatMain.INSTANCE.moduleManager.moduleBy(Blur.class).unableModule();

        }
        if (CheatMain.INSTANCE.clickGui != null) {
            if (!(mc.currentScreen instanceof ClickGui)) {
                ClickGUI.getInstance().unableModule();
            }
        }

        if (IHook.MC.thePlayer.ticksExisted % 4 == 0) {
