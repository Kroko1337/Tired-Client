...
import beta.tiredb56.module.impl.list.visual.NoHurtCam;
...
public class EntityRenderer implements IResourceManagerReloadListener {
        ...
        if (entity != null && this.mc.theWorld != null) {
            ...
            for (Entity value : list) {
                ...
                try {
                    ...
                    if (this.pointedEntity != null && flag && KillAura.getInstance().isState() && KillAura.getInstance().shouldAttack(KillAura.getInstance().getCurrentEntity(), .3) ? vec3.distanceTo(Objects.requireNonNull(vec33)) > KillAura.getInstance().range.getValueInt() : vec3.distanceTo(Objects.requireNonNull(vec33)) > 3) {
                        this.pointedEntity = null;
    ...
    private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            if (!flag && this.zoom > 1.0F) {
                this.zoom = (float) ((double) this.zoom - 0.025D);
                f /= this.zoom;
            }

            if (Config.zoomMode) {
                this.zoom += 0.00025F;
                if ((double) this.zoom > 2.5D) {
                    this.zoom = 2.5F;
                }

                if ((double) this.zoom > 0.01D) {
                    f /= this.zoom;
                }
            }
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (CheatMain.INSTANCE.moduleManager.findModuleByClass(NoHurtCam.class).isState()) return;
