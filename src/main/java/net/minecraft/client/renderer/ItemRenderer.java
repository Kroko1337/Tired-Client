...
public class ItemRenderer {
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            final boolean shouldDoBlockAnim = mc.thePlayer.getCurrentEquippedItem() != null && mc.thePlayer.getCurrentEquippedItem().getItem() instanceof ItemSword && (KillAura.getInstance().isState()) && KillAura.getInstance().autoBlock.getValue() && KillAura.getInstance().getCurrentEntity() != null &&  KillAura.getInstance().autoBlockMode.getValue().equalsIgnoreCase("fake");

            if (this.itemToRender != null) {
                if (this.itemToRender.getItem() instanceof ItemMap) {
                    this.renderItemMap(abstractclientplayer, f2, f, f1);
                } else if (abstractclientplayer.getItemInUseCount() > 0 || shouldDoBlockAnim) {
