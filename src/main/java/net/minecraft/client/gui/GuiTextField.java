	...
	public void drawTextBox(boolean... glow) {
		if (this.getVisible()) {
			...
			if (s.length() > 0 && flag && j < s.length()) {
				j1 = (int) CheatMain.INSTANCE.fontManager.robotoF.drawStringWithShadow2(s.substring(j), (float) j1, (float) i1, i);
			}

			if (flag1) {
				if (flag2) {
					Gui.drawRect(k1, i1 - 1, k1 + 1, i1 + 1 + CheatMain.INSTANCE.fontManager.notosansF.FONT_HEIGHT, -3092272);
				} else {
					CheatMain.INSTANCE.fontManager.robotoF.drawStringWithShadow2("_", (float) k1, (float) i1, i);
				}
			}

			FHook.fontRenderer4.drawString("test", 1222, 1222, -1);
			if (k != j) {
				int l1 = l + CheatMain.INSTANCE.fontManager.robotoF.getStringWidth(s.substring(0, k));
