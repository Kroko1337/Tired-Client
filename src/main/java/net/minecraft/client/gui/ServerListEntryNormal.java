...
public class ServerListEntryNormal implements GuiListExtended.IGuiListEntry {
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected) {
        ...
        CheatMain.INSTANCE.fontManager.confortaa.drawStringWithColor(this.server.serverName, x + 32 + 3, y + 1, 16777215);
        List<String> list = this.mc.fontRendererObj.listFormattedStringToWidth(this.server.serverMOTD, listWidth - 32 - 2);

        for (int i = 0; i < Math.min(list.size(), 2); ++i) {
            CheatMain.INSTANCE.fontManager.confortaa.drawStringWithColor((String) list.get(i), x + 32 + 3, y + 12 + this.mc.fontRendererObj.FONT_HEIGHT * i, 8421504);
        }
        ...
        CheatMain.INSTANCE.fontManager.confortaa.drawStringWithColor(s2, x + listWidth - j - 15 - 2, y + 1, 8421504);
        int k = 0;
        String s = null;
        int l;
        String s1;
