...
import beta.tiredb56.api.util.gui.LayerRenderer;
...
import beta.tiredb56.module.impl.list.visual.Crossair;
...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (this.showCrosshair() && !CheatMain.INSTANCE.moduleManager.findModuleByClass(Crossair.class).isState()) {
        ...
        if (mc.isSingleplayer()) {
            CheatMain.INSTANCE.discordRPC.setStage(new String[]{"In Singleplayer", "lmfao"});
        } else {
            CheatMain.INSTANCE.discordRPC.setStage(new String[]{"Playing on " + mc.getCurrentServerData().serverIP + " as " + mc.thePlayer.getName(), "lmfao"});
        }
    ...
    public void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        for (Score score1 : collection) {
            ...
            animationDown = AnimationUtil.getAnimationState(animationDown, HudModule.getInstance().yPlus / 2f, 3);

            int k = (int) (j1 - j * FHook.fontRenderer2.FONT_HEIGHT + animationDown);
            int l = 132 - k1 + 2;
            Extension.EXTENSION.getGenerallyProcessor().renderProcessor.drawRect(l1 - 2, k, 2, k + FHook.fontRenderer2.FONT_HEIGHT, ClickGUI.getInstance().colorPickerSetting.ColorPickerC.getRGB());
            ...
            FHook.fontRenderer2.drawString(s1, l1, k + 3, 553648127);
