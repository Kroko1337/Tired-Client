...
import beta.tiredb56.notification.notificationcenter.NotificationCenter;
...
public class GuiNewChat extends Gui {
    ...
    public String text;
    private final CustomFont fontRenderer2B = new CustomFont(new Font("Arial", Font.BOLD, 17), true, true);
                    ...
                    if (chatline != null) {
                        ...
                        if (j1 < 200 || flag) {
                            ...
                            if (l1 > 3) {
                                ...
                                String onlyURL = s.replace("§4", "").replace("§c", "").replace("§6", "").replace("§e", "").replace("§2", "").replace("§f", "").replace("§r", "");

                                int i2 = 0;
                                int j2 = -i1 * 9;

                                i2 -= chatline.x;
                                //  BlurUtil.BLUR.render(Minecraft.getMinecraft().getFramebuffer().framebufferTexture, Minecraft.getMinecraft().getFramebuffer(), 0, 0, 40, 40);
                                GlStateManager.resetColor();
                                GlStateManager.disableBlend(); //-- disabling blend to avoid render bugs

                                //if (onlyURL.contains("youtube")) {
                                //	thumbnailUtil.renderPicture(onlyURL, x + (int) i2, y +  (j2 - 122),250, 150);
                                //	}

                                if (rect) {
                                    Extension.EXTENSION.getGenerallyProcessor().renderProcessor.drawRect(i2, j2 - 7, i2 + l + 4, j2 + 2, Integer.MIN_VALUE);
                                }
                                //	CheatMain.blurUtil.blur(i2, j2 - 9, i2 + l + 4, j2, 12);
                                this.text = this.drawnChatLines.get(i1 + this.scrollPos).getChatComponent().getUnformattedText();

                                //thumbnailUtil.renderPicture(onlyURL, 120, 120, 120, 120);
