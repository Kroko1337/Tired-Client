package beta.tiredb56.api.guis;
...
public class ConfigGui extends GuiScreen implements IHook {
    ...
    @Override
    public void initGui() {
        CheatMain.INSTANCE.configManager.init();
        susAmongus = 0;
        for (Config c : CheatMain.INSTANCE.configManager.configs()) {
            if (CheatMain.INSTANCE.configManager.configs() != null) {
                CheatMain.INSTANCE.configManager.loadNoSet(c);
            }
        }
        super.initGui();
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (CheatMain.INSTANCE.configManager.configs().isEmpty()) {
            dragging = false;
            mouseX = 0;
            mouseY = 0;
        }
        ...
        if (CheatMain.INSTANCE.configManager.activeConfig() != null) {
            FontManager.bebasFBig.drawStringWithShadow("Current Config: " + CheatMain.INSTANCE.configManager.activeConfig().name() + " ", x + widthRect, y + 90, -1);
        }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        for (Config c : CheatMain.INSTANCE.configManager.configs()) {
            ...
            if (isOver) {
                CheatMain.INSTANCE.configManager.load(c.name());
            }
            yAxis += 30;
        }
        sussy = true;
